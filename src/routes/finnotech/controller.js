...
module.exports = new (class extends controller {
    ...
    async verifySMSAuthorization(req, res) {
        ...
        try {
            ...
            if (userCheck.cheque.fin_token && expiry > now) {
                await session.abortTransaction();
                return res.status(400).json({ error: "Token still valid!" });
            }
    ...
    async sayadChequeInquiry(req, res) {
        ...
        try {
            ...
            if (checkSayad && checkSayad.status === "Accepted") {
                ...
                return res
                    .status(302)
                    .json({ message: "این چک قبلا تایید شده است." });
            }
            ...
            if (parsed.status === "DONE") {
                ...
                if (checkSayad) {
                    ...
                    await this.SayadCheque.findByIdAndDelete(
                        checkSayad._id
                    ).session(session);
                ...
                const maxDatePersian = new persianDate(plusMaxDate).format(
                    "YYYYMMDD"
                );
                if (parseInt(maxDatePersian) < parseInt(parsed.dueDate)) {
                    ...
                    return res
                        .status(203)
                        .json({ message: "Cheque paydate expired." });
                }
    ...
    async sayadCancelCheque(req, res) {
        ...
        try {
            ...
            const sayadCheque = await this.SayadCheque.findById(
                sayadId
            ).session(session);
            if (!sayadCheque) {
                await session.abortTransaction();
            ...
            if (sayadCheque.status === "Accepted") {
                ...
                return res
                    .status(400)
                    .json({ message: "Cheque is already accepted!" });
            }

            const agency = await this.Agency.findById(
                sayadCheque.agencyId
            ).lean();
            if (!agency) {
                await session.abortTransaction();
                return res.status(400).json({
    ...
    async getParentShahab(req, res) {
        ...
        try {
            ...
            if (cheque) {
                ...
                let parent = await this.Parent.findOne({ phone }).session(
                    session
                );
                if (!parent) {
        ...
        try {
            ...
            if (expiry > now) {
                ...
                if (!generated_token.token || !generated_token.refresh_token) {
                    ...
                    console.error("Faild to generate sms token!");
                    return { success: false, token: null };
                }

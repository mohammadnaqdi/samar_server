...
module.exports = new (class extends controller {
  ...
  async sumSanads(req, res) {
    try {
      if (req.query.code === undefined || req.query.agencyId === undefined) {
        return this.response({
          res,
          code: 214,
  ...
  async accActionShow(req, res) {
    try {
      ...
      if (isNaN(sanadId) || !agencyId) {
        return res.status(213).json({ msg: "input data is not correct" });
      }
  ...
  async chargeCompanyByAdmin(req, res) {
    ...
    try {
      ...
      const agency = await this.Agency.findById(agencyId).session(session);
      if (!agency) {
        await session.abortTransaction();
        return this.response({
          res,
      ...
      if (!wallet || !charge) {
        ...
        return this.response({
          res,
          code: 400,
          message: "wallet or charge account not found",
        });
      }
  ...
  async unChargeCompanyByAdmin(req, res) {
    ...
    try {
      ...
      await session.abortTransaction();
      console.error("Error in unChargeCompanyByAdmin:", error);
      return res.status(500).json({ error: "Internal Server Error." });
    } finally {
      session.endSession();
    }
  ...
  async gozareshHesabKifePool(req, res) {
    try {
      ...
      if (req.query.agencyId) {
        ...
        wallet = agency.settings.find((obj) => obj.wallet !== undefined).wallet;
      } else {
  ...
  async bnkActionSearch(req, res) {
    try {
      ...
      if (type != 1 && type != 13) {
        ...
        if (infoNum != undefined && infoNum != null) qr.push({ infoNum });
        if (type != undefined && type != null) qr.push({ type });
    ...
    try {
      ...
      if (docSanad) {
        ...
        if (checkHistory) {
          ...
          await this.CheckInfo.findByIdAndUpdate(checkHistory.infoId, {

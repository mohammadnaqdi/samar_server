...
module.exports = new (class extends controller {
    ...
    async chargeCompanyByAdmin(req, res) {
        ...
        try {
            ...
            const agency = await this.Agency.findById(agencyId).session(
                session
            );
            if (!agency) {
                await session.abortTransaction();
    ...
    async removePay(req, res) {
        try {
            ...
            if (
                ...
                req.query.idPay === undefined
            ) {
                return this.response({
                    res,
                    code: 214,
                    message: "sanadId idPay need",
                });
            }
            const sanadId = parseInt(req.query.sanadId);
            const idPay = req.query.idPay;
            ...
            await this.PayQueue.findByIdAndUpdate(idPay, {
                $set: { isSetAuto: false, isPaid: false, setter: req.user._id },
            });

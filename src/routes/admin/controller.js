...
module.exports = new (class extends controller {
        ...
        agency = await this.Agency.findOne(
          {
            $and: [
              ...
              { _id: agencyId },
              {
                $or: [{ admin: user._id }, { users: { $in: user._id } }],
              },
            ],
          },
  ...
  async driverFilters(req, res) {
    try {
      ...
      if (doc === "1") {
        ...
        qr.push({
          $or: [{ pic: "" }, { drivingLicence: "" }, { dLicencePic: "" }],
        });
      }
      if (from != "") {
      ...
      if (bed != "0") {
        ...
        for (var d in allDrivers) {
          ...
          const totalAmount = result[0] === undefined ? 0 : result[0].total;
      ...
      for (var d of allDrivers) {
        const user = await this.User.findById(d.userId, "name lastName phone");
        var qrCar = [];
        if (doc === "1") {
          qrCar.push({
            carModel: { $ne: "" },
  ...
  async studentFilters(req, res) {
    try {
      if (req.body.agencyId === undefined || req.body.agencyId.trim() === "") {
        return this.response({
          res,
          code: 214,
          message: "agencyId need",
      ...
      if (school != null) {
        ...
        const onlySchool = await this.School.find({ agencyId }, "").distinct(
          "_id"
        );
      ...
      for (var i in services) {
        ...
        for (var s in services[i].student) {
          ...
          if (student) {
            ...
            studentInfo.physicalConditionDesc = student.physicalConditionDesc;
          ...
          if (parent) {
            studentInfo.parentName = parent.name + " " + parent.lastName;
  ...
  async searchListAccForAdmin(req, res) {
    try {
      ...
      for (var code of levelDets) {
        ...
        // if (isNumericString(search.trim())) {
        //   qr2.push({
        //     $or: [
        //       { code: search.trim() },
        //       { codeLev3: search.trim() },
        //       { codeLev2: search.trim() },
        //       { codeLev1: search.trim() },
        //     ],
        //   });
        // }
        console.log("qr2", qr2);
